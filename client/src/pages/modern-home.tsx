...
export default function ModernHome() {
  ...
  const upcomingToday: UpcomingItem[] = comingSoonData?.results?.slice(0, 3).map((show: any) => ({
    id: show.id,
  ...
  const friendsWatching: FriendActivity[] = friendActivities?.slice(0, 5).map((activity: any, index: number) => ({
    id: activity.id,
    friend: { 
                ...
                <div className="bg-gradient-to-r from-teal-600/20 via-cyan-600/20 to-blue-600/20 rounded-xl p-6 border border-teal-500/20">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      {trendingData?.results?.[0]?.streamingPlatforms && trendingData.results[0].streamingPlatforms.length > 0 && (
                        <div className="flex items-center gap-1 mb-3">
                          {trendingData.results[0].streamingPlatforms.slice(0, 3).map((platform: any, index: number) => (
                            <div key={index} className="w-5 h-5 rounded bg-white p-0.5 flex-shrink-0">
                              {platform.logo_path ? (
                                <img 
                          ...
                          {trendingData.results[0].streamingPlatforms.length > 3 && (
                            <span className="text-[10px] text-gray-400">+{trendingData.results[0].streamingPlatforms.length - 3}</span>
                          )}
                        </div>
                      )}
                ...
                {continueWatching.slice(0, 4).map((item) => (
                  <ContentCard
                    ...
                    item={{
                      id: item.id,
                      title: item.title,
                      poster_path: item.posterPath.replace('https://image.tmdb.org/t/p/w300', ''),
                      vote_average: 0,
